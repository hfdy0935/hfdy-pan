        ...
        <a-form :model="data" class="p-4" :rules="rules" ref="formRef">
            ...
            <a-form-item field="emailCheckCode" hide-label>
                ...
                <a-button type="primary" @click="getEmailCheckCode(data.email, 0)" v-if="showEmailBtn"
                    :disabled="restSeconds > 0" style="height: 36px; min-width: 100px">{{ restSeconds > 0 ? restSeconds
                        + 's' : '发送验证码' }}
                </a-button>
            </a-form-item>
...
import { IconEmail, IconLock, IconSend, IconUser } from '@arco-design/web-vue/es/icon';
...
const isLoading = defineModel<boolean>(); // 是否加载中
...
// 发送邮箱验证码
const { restSeconds, getEmailCheckCode, formRef: formRef1, showEmailBtn } = useEmailCheckCode();
// 提交
const submit = async () => {
    // 校验表单
    let pass = true;
    await formRef1.value?.validate(err => {
        if (err) pass = false;
    });
    if (!pass) return;
